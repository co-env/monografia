...
Os dados coletados são armazenados em um banco de dados,  disponíveis para análise a longo prazo. Soluções com armazenamento em nuvem são boas alternativas, pois possibilitam o acesso remotamente, além de garantir maior robustez.

Foi desenvolvido um painel de dados (\textit{data dashboard}) para que essas medições pudessem ser visualizadas de forma simplificada. 
...
De acordo com a mesma pesquisa da Aspencore \cite{embedded-market-study} já citada anteriormente, 61\% dos projetos de sistemas embarcados usam processadores de 32-bits, e 65\% utiliza algum tipo de sistema operacional. 
