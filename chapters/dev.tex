...
A partir do esquema elétrico foi feito um desenho de PCB (\textit{Printed Circuit Board}, ou Placa de Circuito Impresso), \textit{Single Layer}, e dimensões finais 60x70 mm. As visões 2D e 3D da placa são mostradas na figura \ref{fig:pcb-altium}.
...
\begin{figure}[ht]
	...
	\begin{subfigure}{0.5\textwidth}
	  ...
	  \label{fig:sub2}
	\end{subfigure}
	\caption{Design da PCB no Altium Designer}
	\label{fig:pcb-altium}
\end{figure}

\newpage
A placa mostrada na figura \ref{fig:pcbs} foi fabricada em fibra por uma fresadora CNC e montada usando barras de pinos. O procedimento foi o mesmo para as placas dos demais dispositivos da rede. 
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Placa fabricada e montada}
	\label{fig:pcbs}
\end{figure}
...
O endereçamento dos registradores de aplicação, como os que armazenam os dados coletados e modos de operação se dá de modo virtual. O \textit{datasheet} descreve a lógica de escrita e leitura nesses registradores virtuais, demonstradas nas figuras \ref{fig:as7262_rotina_escrita} e \ref{fig:as7262_rotina_leitura}, respectivamente.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.2]{as7262_rotina_escrita.png}
	\caption{Fluxograma da rotina de escrita em um registrador virtual do sensor AS7262}
	\label{fig:as7262_rotina_escrita}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.2]{as7262_rotina_leitura.png}
	\caption{Fluxograma da rotina de leitura de um registrador virtual do sensor AS7262}
	\label{fig:as7262_rotina_leitura}
\end{figure}
...
Portanto, para realizar interações com o sensor, o usuário deve realizar os passos ilustrados na figura \ref{fig:sgp30-read-write}, utilizando o comando desejado.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.2]{sgp30_read_write.png}
	\caption{Fluxograma da rotina de execução de um comando para interface com o sensor SGP30}
	\label{fig:sgp30-read-write}
\end{figure}
...
Para a iteração com os usuários que fornecerão ao dispositivo seu feedback sobre o ambiente, foi elaborado um firmware cuja arquitetura é mostrada na figura \ref{fig:feedback-arch}. 

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.8]{feedback-arch}
	\caption{Arquitetura do Firmware de Coleta de Feedback}
	\label{fig:feedback-arch}
\end{figure}

O firmware de feedback utiliza um \textit{Timer} para controlar quando deve ocorrer uma coleta de feedback, a API de GPIO para controle dos botões e uma biblioteca para controle do display. 

%Display
Como biblioteca para o \textit{display} SSD1306, utilizamos um componente desenvolvido por Tara K \cite{TaraK} baseado no ESP-IDF \cite{SSD1306}. Este utiliza uma porta I²C diferente da utilizada pelos sensores para que não haja uma competição desse recurso entre as \textit{tasks} executadas em paralelo no sistema. Em \textit{display.c} desenvolvemos as telas utilizadas durante a rotina de coleta de \textit{feedback}, mostradas na figura \ref{fig:telas-feedback}.
...
\begin{figure}[h]
	...
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{pergunta-temp.jpeg}
		\caption{Conforto térmico}
	\end{subfigure}%
	\hfill
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{pergunta-temp2.jpeg}
		\caption{Detalhes sensação térmica}
	\end{subfigure}%
	\hfill
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{pergunta-som.jpeg}
		\caption{Conforto acústico}
	\end{subfigure}%
	\hfill
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{pergunta-luz.jpeg}
		\caption{Conforto luminoso}
	\end{subfigure}
	\hspace{0.05\textwidth}
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[width=\textwidth]{pergunta-luz2.jpeg}
		\caption{Detalhes sobre desconforto luminoso}
	\end{subfigure}
	\caption{Telas com as perguntas do feedback}
	\label{fig:telas-feedback}
\end{figure}
% \newpage
...
Por fim, as perguntas associadas às medições da luz são "Está confortável com a luz?", e caso a resposta seja 'Não', "Muito claro?" para saber se o desconforto está relacionado a uma alta ou baixa intensidade da luz. 

A tarefa de feedback inicialmente espera por um evento gerado por um \textit{timer} ser colocado na fila de eventos adequada, identificando que deve ser realizada a coleta. A rotina de coleta segue, então, uma \textbf{máquina de estados} que foi implementada seguindo um \textit{table-driven approach}, mostrada na figura \ref{fig:fsm-feedback}. Os estados são determinados de acordo com as respostas às perguntas de conforto mostradas anteriormente na figura \ref{fig:telas-feedback}. 
...
No início de uma coleta, é executado o estado inicial e a tarefa volta a esperar por um novo evento, que pode ser causado por um \textit{timeout} ou um GPIO. Caso seja um GPIO, um novo estado é executado; caso seja um \textit{timer} e o seu ID corresponder ao de \textit{Timeout}, a coleta é encerrada. Como essa coleta é iniciada periodicamente, a fim de economizar energia quando o usuário não estiver próximo ao dispositivo, ou não queira responder no momento, após 1 minuto sem resposta a coleta é encerrada automaticamente. 
...
Em cada um dos estados intermediários, a resposta dos botões é armazenada, a tela é atualizada para uma nova pergunta, e o \textit{timer} TIMEOUT é reiniciado. Quando ocorre um \textit{timeout}, é executando o estado FINAL. 
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.2]{exemplo-envio-mqtt.png}
	\caption{Exemplo de envio de um bloco de dados ao servidor}
	\label{fig:exemplo-envio-mqtt}
\end{figure}

Durante uma rotina de envio de dados ao servidor, o dispositivo publica cinco mensagens em seguida no mesmo tópico, sendo essas mensagens atreladas sempre ao mesmo dispositivo. Cada uma das mensagens contém dados de um dos sensores, ou seja, teremos uma mensagem com valores de temperatura e umidade do BME280, uma com valores de TVOC e $CO_{2}$ do SGP30, uma com valores das 6 cores e a intensidade da luz do ambiente do AS7262 e uma com o valor de ruído ambiente captado pelo microfone, além de uma última mensagem com as respostas de \textit{feedback} do usuário, exemplificados na figura \ref{fig:exemplo-envio-mqtt}. Essa rotina de envio acontece toda vez que o dispositivo \textit{gateway} recebe dados via BLE Mesh ou quando uma medição dos seus próprios sensores é realizada. Todas essas mensagens seguem o formato a seguir, no qual \textbf{device\_name} se refere ao nome do dispositivo ao qual essa medição pertence, \textbf{sensor\_name} é o nome do sensor que realizou a medição e, por último, os valores de cada parâmetro lido:
